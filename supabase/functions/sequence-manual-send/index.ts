...
function jsonResponse(status, body, req) {
...
const OPEN_DETECTION_WINDOW_MS = Math.max(CHECK_INTERVAL_MS, parseInt(Deno.env.get('SEQUENCE_OPEN_WINDOW_MS') || '', 10) || 10 * 60 * 1000);
function log(...args) {
...
async function loadSequenceContext(sequenceId) {
  const { data: steps, error } = await supabase.from('sequence_steps').select('*').eq('sequence_id', sequenceId).is('archived_at', null).order('step_number', {
    ascending: true
  });
...
async function evaluateNodeEligibility({ sequenceId, contactId, step }) {
  ...
  if (!eligible && branch === 'opened' && sent) {
    ...
    const list = siblings || [];
    if (list.length > 0 && list.every((s)=>s.parent_branch === BRANCH_OPENED)) {
      eligible = true;
    }
  }
  return {
...
/** Ensure a step has its batch queue row (the runner also does this lazily). */ async function ensureStepBatchState(sequence, step) {
  ...
  try {
    ...
    logErr(`Failed to create batch state for step ${step.id}: ${error.message}`);
  }
  return loadStepBatchState(sequence.id, step.id);
}
async function manualBatchGate(sequence, step) {
...
/** Call AFTER a provider-confirmed manual send so the queue stays in sync with the runner. */ async function recordStepBatchSend(sequence, step) {
...
async function sendStepEmail({ enrollment, step, emailType, contact, attachments }) {
  ...
  const { data: logRow, error: createError } = await supabase.from('email_logs').insert({
    campaign_id: null,
  ...
  if (createError) {
    if (createError.code === '42703') {
      const { data: logRow2, error: createError2 } = await supabase.from('email_logs').insert({
        campaign_id: null,
        contact_id: enrollment.contact_id,
        email: contact.email,
        status: 'pending',
...
async function sendAndUpdate(enrollment, subjectLine, personalizedHtml, plainText, logRow, trackingId, contact, attachments) {
  ...
  try {
    ...
    try {
      ...
      log(`Failed to mark email log ${logRow.id} failed after send error: ${updateError.message}`);
    }
    throw error;
  }
}
...
async function advanceAfterSend({ enrollment, sequence, contact }) {
  ...
  const currentStep = steps.find((s)=>s.id === enrollment.current_step_id) || null;
  if (!currentStep) {
...
async function ensureEnrollmentAndAdvance({ sequenceId, contactId, sequence, contact, step }) {
  const nowIso = new Date().toISOString();
  const { data: existing } = await supabase.from('sequence_enrollments').select('id').eq('sequence_id', sequenceId).eq('contact_id', contactId).maybeSingle();
  let enrollment;
  if (existing) {
...
function appendOpenPixel(html, emailLogId, contactEmail, trackingId) {
...
Deno.serve(async (req)=>{
  ...
  const respond = (status, body)=>jsonResponse(status, body, req);
  try {
    ...
    const contactIds = [
      ...new Set(rawContactIds.map((v)=>String(v).trim()).filter(Boolean))
    ];
    const { data: sequence, error: seqError } = await supabase.from('sequences').select('*').eq('id', sequenceId).maybeSingle();
    if (seqError) throw toError(seqError, 'Failed to fetch sequence');
    if (!sequence) return respond(404, {
      success: false,
    ...
    if (!step) return respond(404, {
      success: false,
      error: `Step not found: ${stepId}`
    });
    // Enrolled contacts (for contact lookups).
    const { data: enrolled } = await supabase.from('sequence_enrollments').select('contact_id, contacts(*)').eq('sequence_id', sequenceId);
    const recipientById = new Map();
    for (const row of enrolled || []){
      if (row.contacts) recipientById.set(row.contact_id, row.contacts);
    }
    // Already sent for this node (duplicate protection).
    const { data: sentLogs, error: sentLogsError } = await supabase.from('sequence_step_logs').select('contact_id').eq('sequence_id', sequenceId).eq('sequence_step_id', stepId);
    if (sentLogsError) throw toError(sentLogsError, 'Failed to fetch sent step logs');
    const alreadySent = new Set((sentLogs || []).map((l)=>l.contact_id));
    ...
    const branchAttachments = branchStepId != null ? await loadAndDownloadBranchStepAttachments(branchStepId) : [];
    const attachments = [
      ...stepAttachments,
      ...branchAttachments
    ];
    ...
    for (const contactId of contactIds){
      ...
      const { data: raceLog } = await supabase.from('sequence_step_logs').select('id').eq('sequence_id', sequenceId).eq('sequence_step_id', step.id).eq('contact_id', contactId).maybeSingle();
      if (raceLog) {
      ...
      try {
        ...
        const { log: emailLog } = await sendStepEmail({
          enrollment,
          step,
          emailType,
          contact,
        ...
        await ensureEnrollmentAndAdvance({
          sequenceId,
          contactId,
          sequence,
          contact,
        ...
        logErr(`Manual send failed for contact ${contactId} (step ${step.step_number}): ${error.message}`);
        results.push({
          contact_id: contactId,
          status: 'failed',
          error: error.message
    ...
    const scheduled = results.filter((r)=>r.status === 'increment_scheduled' || r.status === 'batch_scheduled').length;
    ...
    logErr(`Manual send failed: ${error.message}`);
