...
const RETRY_DELAYS = [
  ...
  60,
  120
]; // seconds — matches the backend worker
 ...
 */ function computeNextRun(schedule) {
  ...
  const instant = (y, mo, d)=>istDateTimeToUtc(`${y}-${String(mo + 1).padStart(2, '0')}-${String(d).padStart(2, '0')}`, schedule.send_time);
  if (schedule.schedule_type === 'one_time') {
    return instant(Number(match[1]), Number(match[2]) - 1, Number(match[3]));
  }
  if (schedule.schedule_type === 'weekly') {
    const days = Array.isArray(schedule.weekly_days) ? schedule.weekly_days : String(schedule.weekly_days || '').split(',').map((s)=>s.trim()).filter(Boolean);
 ...
 */ function isCampaignDue(campaign, schedule, nowMs) {
...
async function getCampaignSchedule(campaignId) {
  const { data, error } = await supabase.from('campaign_schedules').select('*').eq('campaign_id', campaignId).eq('is_active', true).maybeSingle();
 ...
 */ function isBatchedCampaignDue(campaign, schedule, nowMs) {
 ...
 */ async function getDueCampaigns() {
  const { data, error } = await supabase.from('campaigns').select('*, campaign_schedules(*)').eq('status', 'scheduled');
  if (error) throw error;
  const now = Date.now();
  const due = [];
  for (const campaign of data || []){
    const schedules = (campaign.campaign_schedules || []).filter((s)=>s && s.is_active !== false);
...
async function getStuckSendingCampaigns() {
  const cutoff = new Date(Date.now() - RECLAIM_AFTER_MS).toISOString();
  const { data, error } = await supabase.from('campaigns').select('*').eq('status', 'sending').lt('updated_at', cutoff);
  if (error) throw error;
  return data || [];
}
/**
...
const NON_DELIVERABLE_EMAIL_RE = /(^__)|@example\.(com|org|net|edu)$|\.(test|invalid|localhost|local)$/i;
function isDeliverableRecipientEmail(email) {
  const value = String(email || '').trim();
  if (!value) return false;
  if (!EMAIL_REGEX.test(value)) return false;
  return !NON_DELIVERABLE_EMAIL_RE.test(value);
}
async function resolveContactsForCampaign(campaignId, audienceSegment) {
  ...
  const { data: configs, error: cfgError } = await supabase.from('campaign_followups').select('campaign_id, trigger_type').eq('followup_campaign_id', campaignId).limit(1);
  ...
  const notOpenedAudience = !!configs && !!configs[0] && configs[0].trigger_type === 'not_opened';
  const openedAtByContact = new Map();
  if (sourceCampaignId) {
    const openedQuery = supabase.from('email_logs').select('contact_id, opened_at').eq('campaign_id', sourceCampaignId);
    ...
    const audienceLogs = notOpenedAudience ? await openedQuery.not('opened', 'is', true).not('clicked', 'is', true) : await openedQuery.eq('opened', true);
    ...
    const openedIds = Array.from(new Set((audienceLogs.data || []).map((r)=>r.contact_id)));
    if (openedIds.length === 0) {
      valid = [];
    } else {
      const { data: rows, error: contactsError } = await supabase.from('contacts').select('*').in('id', openedIds);
      if (contactsError) throw new Error(`Failed to fetch follow-up contacts: ${contactsError.message}`);
      valid = (rows || []).filter((c)=>isDeliverableRecipientEmail(c.email));
    }
    if (valid.length === 0) {
  ...
  if (valid.length > 0) {
    const { data: existing, error: existingError } = await supabase.from('campaign_contacts').select('contact_id').eq('campaign_id', campaignId);
    ...
    const newRows = valid.filter((c)=>!existingIds.has(c.id)).map((c)=>({
...
// ─── email_logs helpers ────────────────────────────────────────────────────
async function createEmailLogs(campaignId, contacts) {
  ...
  if (error) {
    ...
    if (error.code === '42703') {
      const { error: err2 } = await supabase.from('email_logs').insert(rows.map(({ tracking_id: _t, ...rest })=>rest));
...
async function getLogsByCampaign(campaignId) {
  const { data, error } = await supabase.from('email_logs').select('*').eq('campaign_id', campaignId);
 ...
 */ async function claimPendingLogs(campaignId, limit) {
  const nowIso = new Date().toISOString();
  // Step 1 — deterministically select the IDs to claim (SELECT supports limit).
  let select = supabase.from('email_logs').select('id').eq('campaign_id', campaignId).eq('status', 'pending').or(`next_retry_at.is.null,next_retry_at.lte.${nowIso}`);
  ...
  const { data, error } = await supabase.from('email_logs').update({
    status: 'sending',
    last_attempt_at: nowIso
  }).in('id', ids).eq('status', 'pending').select('*');
  if (error) throw new Error(`Failed to claim pending logs: ${error.message}`);
  return data || [];
}
/** Reclaim logs stuck in 'sending' by a crashed invocation (>10 min). */ async function recoverStuckLogs(campaignId) {
  const cutoff = new Date(Date.now() - RECLAIM_AFTER_MS).toISOString();
  const { error } = await supabase.from('email_logs').update({
    status: 'pending'
  }).eq('campaign_id', campaignId).eq('status', 'sending').lt('last_attempt_at', cutoff);
  if (error) throw new Error(`Failed to recover stuck logs: ${error.message}`);
}
/** On budget exit, release logs WE claimed (last_attempt_at <= our claim time). */ async function releaseClaimedLogs(campaignId, claimTimeIso) {
  const { error } = await supabase.from('email_logs').update({
    status: 'pending'
  }).eq('campaign_id', campaignId).eq('status', 'sending').lte('last_attempt_at', claimTimeIso);
 ...
 */ async function finalizeCampaign(campaignId, updates) {
...
async function getLogsStats(campaignId) {
 ...
 */ async function recordFollowupDelivery(followupCampaignId, sourceCampaignId, contactId, email, openedAt) {
  const sentAt = new Date().toISOString();
  const { error } = await supabase.from('campaign_followup_logs').upsert({
...
async function syncCampaignAnalytics(campaignId) {
  ...
  const { error } = await supabase.from('campaign_analytics').upsert({
    ...
    open_rate: stats.delivered > 0 ? Number((stats.opened / stats.delivered * 100).toFixed(1)) : 0,
    click_rate: stats.delivered > 0 ? Number((stats.clicked / stats.delivered * 100).toFixed(1)) : 0
  }, {
...
/** Load the campaign's attachment metadata rows from Supabase. */ async function loadCampaignAttachments(campaignId) {
...
async function fetchCampaignTemplateHtml(templateId) {
  const { data: tpl, error } = await supabase.from('templates').select('*').eq('id', templateId).maybeSingle();
  if (error) {
...
async function sendOneEmail(emailLog, campaign, contactMap, index, total, attachments = []) {
  ...
  const decoded = decodeHtmlEntities(personalizeTemplate(bodyHtml, contact, emailLog.email));
...
async function processCampaign(campaignId) {
  ...
  try {
    ...
    try {
      ...
      throw new Error(`Campaign ${campaignId} could not attach its files: ${message}`, {
        cause: error
      });
    }
    ...
    while(sent + failed < runCap){
      ...
      for(let i = 0; i < batch.length; i++){
        ...
        try {
          ...
          if (sourceCampaignId) {
            await recordFollowupDelivery(campaignId, sourceCampaignId, String(logRow.contact_id), String(logRow.email || ''), openedAtByContact.get(String(logRow.contact_id)) || null);
          }
          sent++;
        } catch (error) {
          const message = error instanceof Error ? error.message : String(error);
    ...
    try {
      ...
      logErr(`Analytics sync failed (non-fatal): ${analyticsError.message}`);
    }
    if (stats.pending > 0) {
      ...
      if (isValidBatch) {
        ...
        log(`Campaign ${campaignId} is batched — sent this batch; ` + `next batch at ${nextAt} (${Number(gapHours) || 0} hour(s) wait).`);
        return {
          sent,
          failed,
          total: stats.total
      ...
      log(`Campaign ${campaignId} still has ${stats.pending} pending recipient(s) — ` + `returned to "scheduled" so the next cron tick continues.`);
    } else {
      ...
      const isRecurring = schedule && (schedule.schedule_type === 'weekly' || schedule.schedule_type === 'monthly');
      if (isRecurring) {
        const next = computeNextRun(schedule);
        if (next) {
          const nowIso = new Date().toISOString();
          const { error: schedError } = await supabase.from('campaign_schedules').update({
          ...
          log(`Campaign ${campaignId} recurring — ${stats.delivered} delivered this occurrence; ` + `advanced to next run ${next.toISOString()} and kept "scheduled".`);
          return {
            sent,
            failed,
            total: stats.total
    ...
    if (delivered > 0) {
      ...
      logErr(`Campaign ${campaignId} marked "failed": ${error.message}`);
    }
    throw error;
  }
}
  ...
  try {
    ...
    for (const campaign of due){
      try {
        ...
        summary.failed++;
        logErr(`Campaign ${campaign.id} failed: ${error.message}`);
      }
    }
    ...
    for (const campaign of stuck){
      try {
        ...
        logErr(`Recovery failed for campaign ${campaign.id}: ${error.message}`);
      }
    }
    ...
    logErr(`Scheduler tick failed: ${error.message}`);
    return new Response(JSON.stringify({
      success: false,
      error: error.message
    }), {
