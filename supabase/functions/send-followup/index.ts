...
/** Always-reachable open pixel handled by the existing campaign-tracker function. */ function appendEdgeTrackingPixel(html, campaignId, contactEmail, trackingId) {
...
/** RFC 2047 encoded-word for non-ASCII headers (Subject / display name). */ function encodeHeader(value) {
...
async function getCampaign(campaignId) {
  const { data, error } = await supabase.from('campaigns').select('id, campaign_name, subject_line, from_name, audience_segment, campaign_type, email_body, html_content, template_name, status, mailchimp_campaign_id, recipient_count, sent_at, scheduled_at, created_at, updated_at').eq('id', campaignId).maybeSingle();
 ...
 */ async function isScheduledFollowup(followupCampaignId) {
  if (!followupCampaignId) return false;
  const { data: schedule, error: scheduleError } = await supabase.from('campaign_schedules').select('id').eq('campaign_id', followupCampaignId).limit(1);
  if (scheduleError) return false;
  if (!schedule || schedule.length === 0) return false;
  const { data: campaign, error: campaignError } = await supabase.from('campaigns').select('status').eq('id', followupCampaignId).maybeSingle();
  if (campaignError) return false;
  return campaign && String(campaign.status).toLowerCase() === 'scheduled';
}
async function isNotOpenedAudience(followupCampaignId) {
  if (!followupCampaignId) return false;
  const { data: config, error } = await supabase.from('campaign_followups').select('trigger_type').eq('followup_campaign_id', followupCampaignId).limit(1);
  if (error) return false;
  return !!config && config[0]?.trigger_type === 'not_opened';
}
/** Load the follow-up campaign's attachment metadata rows from Supabase. */ async function loadCampaignAttachments(campaignId) {
  const { data, error } = await supabase.from('campaign_attachments').select('*').eq('campaign_id', campaignId).order('created_at', {
    ascending: true
  });
  if (error) {
    if (error.code === '42P01') {
 ...
 */ async function loadAndDownloadAttachments(campaignId) {
...
async function getFollowupLog(campaignId, contactId, followupCampaignId) {
  const { data, error } = await supabase.from('campaign_followup_logs').select('*').eq('campaign_id', campaignId).eq('contact_id', contactId).eq('followup_campaign_id', followupCampaignId).maybeSingle();
  if (error) throw new Error(`Failed to check for an existing follow-up: ${error.message}`);
  return data || null;
}
async function insertFollowupLog(fields) {
  const { data, error } = await supabase.from('campaign_followup_logs').insert({
...
async function updateFollowupLog(id, updates) {
  const { data, error } = await supabase.from('campaign_followup_logs').update(updates).eq('id', id).select('*').maybeSingle();
  if (error) throw new Error(`Failed to update follow-up record: ${error.message}`);
  return data || null;
}
async function getOrCreateEmailLog(campaignId, contactId, email) {
  const base = {
    campaign_id: campaignId,
  ...
  const { data: existing, error: existingError } = await supabase.from('email_logs').select('*').eq('campaign_id', campaignId).eq('contact_id', contactId).maybeSingle();
  if (!existingError && existing) return existing;
  const { data, error } = await supabase.from('email_logs').insert(base).select('*').maybeSingle();
  if (error) {
    if (error.code === '23505') {
      const { data: again, error: againError } = await supabase.from('email_logs').select('*').eq('campaign_id', campaignId).eq('contact_id', contactId).maybeSingle();
    ...
    if (error.code === '42703') {
      ...
      const { data: fallback, error: fallbackError } = await supabase.from('email_logs').insert(rest).select('*').maybeSingle();
...
async function linkContactToCampaign(campaignId, contactId) {
...
async function finalizeFollowupCampaign(followupCampaignId) {
  try {
    await syncCampaignAnalytics(followupCampaignId);
    const { data: logs, error } = await supabase.from('email_logs').select('id').eq('campaign_id', followupCampaignId);
    ...
    logErr(`Finalize follow-up campaign ${followupCampaignId} failed (non-fatal): ${error.message}`);
  }
}
// ─── Opened-recipient verification (mirrors followupService) ───────────────
async function getOpenedByContact(campaignId, isAll, contactIds, notOpenedAudience) {
  const byContact = new Map();
  if (contactIds.length === 0) return byContact;
  let query = supabase.from('email_logs').select('contact_id, email, opened_at, campaign_id').in('contact_id', contactIds);
...
async function sendFollowupEmail(followupCampaignId, contactId, email, attachments = []) {
  ...
  try {
    ...
    log(`Contact ${contactId} not found — using email from the open event: ${error.message}`);
  }
  contact = contact || {};
  ...
  try {
    ...
    log(`Could not link follow-up recipient to campaign_contacts: ${error.message}`);
  }
  const decoded = decodeHtmlEntities(personalizeTemplate(followupCampaign.html_content || '', contact, email));
...
// ─── Action handlers ───────────────────────────────────────────────────────
async function handleSendSelected(payload) {
  ...
  const contactIds = Array.isArray(payload.contact_ids) ? payload.contact_ids.map((id)=>String(id)) : [];
  if (contactIds.length === 0) {
    throw new Error('Select at least one opened contact');
  }
  const followupCampaignId = payload.followup_campaign_id ? String(payload.followup_campaign_id) : '';
  ...
  let batchCap = null;
  try {
    const { data: batchCfg, error: batchErr } = await supabase.from('campaigns').select('send_in_batches, batch_size').eq('id', followupCampaignId).maybeSingle();
  ...
  try {
    ...
    log(`Could not resolve contact names: ${error.message}`);
  }
  const contactById = new Map(contacts.map((c)=>[
      String(c.id),
      c
    ]));
  ...
  const mimeAttachments = await loadAndDownloadAttachments(followupCampaignId);
  const results = [];
  let sentCount = 0;
  for (const contactId of idsToSend){
    ...
    try {
      ...
      if (logRow && logRow.id) {
        await updateFollowupLog(logRow.id, {
          status: 'sent',
          sent_at: sentAt
        });
      } else {
        ...
        if (refetched && refetched.id) {
          await updateFollowupLog(refetched.id, {
            status: 'sent',
            sent_at: sentAt
          });
        }
...
async function handleSendPending(pendingId) {
  const { data: pending, error } = await supabase.from('campaign_followup_logs').select('*').eq('id', pendingId).maybeSingle();
  ...
  const audienceCheck = supabase.from('email_logs').select('contact_id').eq('campaign_id', pending.campaign_id).eq('contact_id', pending.contact_id);
  const openedCheck = pendingNotOpenedAudience ? await audienceCheck.not('opened', 'is', true).not('clicked', 'is', true).limit(1) : await audienceCheck.eq('opened', true).limit(1);
  const openedCheckError = openedCheck.error;
  if (openedCheckError) {
    throw new Error(`Failed to verify the recipient ${pendingNotOpenedAudience ? 'did not open' : 'opened'} the original campaign: ${openedCheckError.message}`);
  }
  if (!openedCheck.data || openedCheck.data.length === 0) {
    throw new Error(pendingNotOpenedAudience ? 'Recipient opened the original campaign — not-opened follow-up not sent' : 'Recipient did not open the original campaign — follow-up not sent');
  }
  ...
  const mimeAttachments = await loadAndDownloadAttachments(pending.followup_campaign_id);
  try {
    await sendFollowupEmail(pending.followup_campaign_id, pending.contact_id, pending.email, mimeAttachments);
    const sentAt = new Date().toISOString();
    await updateFollowupLog(pending.id, {
      status: 'sent',
      sent_at: sentAt
    });
    await recordFollowupHistory({
    ...
    try {
      ...
      logErr(`Could not mark pending follow-up failed: ${innerError.message}`);
    }
    throw error;
  }
}
...
async function handleSyncPending() {
  const { data: configs, error } = await supabase.from('campaign_followups').select('*').eq('is_active', true);
  ...
  for (const config of configs || []){
    ...
    const audienceQuery = supabase.from('email_logs').select('contact_id, email, opened_at').eq('campaign_id', originalCampaignId);
    const audienceResult = await (syncNotOpenedAudience ? audienceQuery.not('opened', 'is', true).not('clicked', 'is', true) : audienceQuery.eq('opened', true));
    ...
    for (const openedLog of openedLogs || []){
      ...
      try {
        ...
        if (existing && [
          ...
          'already_sent',
          'pending'
        ].includes(existing.status)) continue;
        ...
        logErr(`sync_pending: failed for ${contactId}: ${error.message}`);
      }
    }
  }
  return {
